...
var te = window.previewer = {
  ...
  dotnetInterop: {
    ...
    previewLinkNavigation: async function(url, rawHref) {
      // HACK: Must call sync for result - WPF code uses async so potential lockup scenario
...
$(document).ready(function () {
    ...
    function navigateHash(hash) {
        ...
        const sel = "a[name='" + safeHash + "'],#" + safeHash;
...
$(document).on("contextmenu",
    function (e) {
        var parm = { Top: 1, Left: 1, Id: '', Type: '', Src: '', Href: '', TextSelection: '', NodeContent: '' };
        ...
        if (e.target) {
            ...
            if (e.target.nodeName === "svg")
                parm.NodeContent = e.target.outerHTML;
        }
